...
class ProxyChecker:
    ...
    @staticmethod
    def _save_proxies(proxy_type: str, live_proxies: list, without_proxy_type: bool):
        try:
            ...
            with open(filename, "w", encoding="utf8") as file:
                if without_proxy_type:
                    file.write(
                        "\n".join(proxy.split("://", 1)[-1] for proxy in live_proxies)
                    )
                else:
                    file.write("\n".join(live_proxies))
    ...
    def time_taken(started: float):
        ...
        if elapsed < 1:
            format_elapsed = (
                f"{Colors.LBLUE}{round(elapsed * 1000)}{Colors.END} milliseconds!"
            )
        elif elapsed < 60:
    ...
    def _get_default_concurrent(proxy_count: int):
        ...
        return next(
            (value for upper_bound, value in ranges if proxy_count <= upper_bound), 300
        )
    ...
    async def _fetch_proxies(self, proxy_shuffle: bool):
        try:
            async with AsyncClient() as client:
                ...
                logging.info(
                    f"Found {len(unique_proxies)} {self.proxy_type.upper()} proxies!"
                )
                print(
                    f"Found {Colors.BYELLOW}{len(unique_proxies)} {Colors.LPURPLE}{self.proxy_type.upper()}{Colors.END} proxies!"
                )

                while True:
                    limiter = input(
                        f'How many proxies should be check? (press "{Colors.CYAN}ENTER{Colors.END}" to skip limit): '
                    )
                    ...
                    logging.info(
                        f"Proxy has been limit to {len(proxies)} for checking."
                    )
                    return proxies
    ...
    async def _check_proxies(self, proxy: str, check_url: str, timeout: float):
        ...
        try:
            async with AsyncClient(
                transport=transport, timeout=timeout, verify=False
            ) as client:
    ...
    async def start_checker(
        ...
        if use_semaphore:
            concurrent = (
                ...
                else self._get_default_concurrent(len(proxies))
            )
            semaphore = asyncio.Semaphore(concurrent)
            logging.info(
                f"Checker started! Concurrent={concurrent}, "
                f"Timeout={timeout}, "
                f"Proxy Shuffle={proxy_shuffle}, "
                f"Without Proxy Type={without_proxy_type}"
            )
            print(
                ...
                f"Without Proxy Type: {Colors.WHITE}{without_proxy_type}{Colors.END}"
            )
        else:
            logging.info(
        ...
        if self.live_proxies:
            self._save_proxies(self.proxy_type, self.live_proxies, without_proxy_type)
        else:
...
def main():
    parser = ArgumentParser(description="Proxy Scraper|Checker")
    parser.add_argument(
        "-cf",
        "--config-file",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=10,
        help="Proxy timeout setting (Default: 10)",
    )
    parser.add_argument(
        "-wpt",
        "--without-proxy-type",
        action="store_true",
        default=False,
        help="Save proxies without their proxy type in the text file content (Default: False)",
    )
    parser.add_argument(
        "-ps",
        "--proxy-shuffle",
        action="store_true",
        default=False,
        help="Proxy shuffle or randomize (Default: False)",
    )
    parser.add_argument(
        "-us",
        "--use-semaphore",
        action="store_true",
        default=False,
        help="Semaphore usage (Default: False)",
    )
    ...
    asyncio.run(
        proxy_tool.start_checker(
            ...
            without_proxy_type=args.without_proxy_type,
        )
    )
